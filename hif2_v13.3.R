...
### V13.2: Zoom part of HR diragam now zooms into HB to see loops
...
#Teff.index <- which.min(taus.2)
#TAMS.index <- which.min(abs(ages-TAMS))
#RGB.TIP.index <- which.min(abs(ages-RGB.TIP))
#AGB.index <- which.min(abs(ages-AGB))
my.index <- which.min(abs(HB_ages-(max(HB_ages)-min(HB_ages))/2))
special.prof.num <- HB_prof_nums[my.index]#age.index]
#special.prof.num <- prof_nums[Teff.index]
...
for (p.i in 1:length(plot.set)) {
    ...
    plot_hif_tau <- function(..., make.x=T, make.y=T, 
        ...
        #tau_min <- round(min(taus[taus!=min(taus)]))
        #ymin <- floor(log10(tau_min))
